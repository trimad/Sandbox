...
{
    public class TredecicMultibrot : Fractal
    {
        private readonly int bailout;

        public TredecicMultibrot(int width, int height, int bailout, int highestExposureTarget)
        {
            name = "TredecicMultibrot";
            this.width = width;
            this.height = height;
        ...
        {
            // Tredecic Multibrot / degree-13 Mandelbrot-family set:
            // z_{n+1} = z_n^13 + c, z_0 = 0.
            // Degree d Multibrots have (d - 1)-fold rotational symmetry in the
            // parameter plane; the tredecic case has twelve compact arms and very fine tendrils.
            Parallel.For(0, width, x =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    while (iterations < bailout && zr * zr + zi * zi <= 4.0)
                    {
                        ...
                        // Compute z^13 by repeated complex multiplication to avoid
