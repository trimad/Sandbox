...
{
    public class HeptadecicMultibrot : Fractal
    {
        private readonly int bailout;

        public HeptadecicMultibrot(int width, int height, int bailout, int highestExposureTarget)
        {
            name = "HeptadecicMultibrot";
            this.width = width;
            this.height = height;
        ...
        {
            // Heptadecic Multibrot / degree-17 Mandelbrot-family set:
            // z_{n+1} = z_n^17 + c, z_0 = 0.
            // Degree d Multibrots have (d - 1)-fold rotational symmetry in the
            // parameter plane; the heptadecic case has sixteen compact arms.
            Parallel.For(0, width, x =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    while (iterations < bailout && zr * zr + zi * zi <= 4.0)
                    {
                        ...
                        // Compute z^17 by repeated complex multiplication to avoid
                        // fragile expanded-polynomial coefficient mistakes.
                        double powerR = zr;
                        double powerI = zi;
                        for (int power = 2; power <= 17; power++)
                        {
