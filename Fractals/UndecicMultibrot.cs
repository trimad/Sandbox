...
{
    public class UndecicMultibrot : Fractal
    {
        private readonly int bailout;

        public UndecicMultibrot(int width, int height, int bailout, int highestExposureTarget)
        {
            name = "UndecicMultibrot";
            this.width = width;
            this.height = height;
        ...
        {
            // Undecic Multibrot / degree-11 Mandelbrot-family set:
            // z_{n+1} = z_n^11 + c, z_0 = 0.
            // Degree d Multibrots have (d - 1)-fold rotational symmetry in the
            // parameter plane; the undecic case has ten compact arms with very fine tendrils.
            Parallel.For(0, width, x =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    while (iterations < bailout && zr * zr + zi * zi <= 4.0)
                    {
                        ...
                        // Compute z^11 by repeated complex multiplication to avoid
                        // fragile expanded-polynomial coefficient mistakes.
                        double powerR = zr;
                        double powerI = zi;
                        for (int power = 2; power <= 11; power++)
                        {
