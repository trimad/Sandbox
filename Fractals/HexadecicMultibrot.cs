...
{
    public class HexadecicMultibrot : Fractal
    {
        private readonly int bailout;

        public HexadecicMultibrot(int width, int height, int bailout, int highestExposureTarget)
        {
            name = "HexadecicMultibrot";
            this.width = width;
            this.height = height;
        ...
        {
            // Hexadecic Multibrot / degree-16 Mandelbrot-family set:
            // z_{n+1} = z_n^16 + c, z_0 = 0.
            // Degree d Multibrots have (d - 1)-fold rotational symmetry in the
            // parameter plane; the hexadecic case has fifteen compact arms.
            Parallel.For(0, width, x =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    while (iterations < bailout && zr * zr + zi * zi <= 4.0)
                    {
                        ...
                        // Compute z^16 by repeated complex multiplication to avoid
