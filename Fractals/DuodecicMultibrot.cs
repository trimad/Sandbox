...
{
    public class DuodecicMultibrot : Fractal
    {
        private readonly int bailout;

        public DuodecicMultibrot(int width, int height, int bailout, int highestExposureTarget)
        {
            name = "DuodecicMultibrot";
            this.width = width;
            this.height = height;
        ...
        {
            // Duodecic Multibrot / degree-12 Mandelbrot-family set:
            // z_{n+1} = z_n^12 + c, z_0 = 0.
            // Degree d Multibrots have (d - 1)-fold rotational symmetry in the
            // parameter plane; the duodecic case has eleven compact arms and fine tendrils.
            Parallel.For(0, width, x =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    while (iterations < bailout && zr * zr + zi * zi <= 4.0)
                    {
                        ...
                        // Compute z^12 by repeated complex multiplication to avoid
