...
{
    public class QuindecicMultibrot : Fractal
    {
        private readonly int bailout;

        public QuindecicMultibrot(int width, int height, int bailout, int highestExposureTarget)
        {
            name = "QuindecicMultibrot";
            this.width = width;
            this.height = height;
        ...
        {
            // Quindecic Multibrot / degree-15 Mandelbrot-family set:
            // z_{n+1} = z_n^15 + c, z_0 = 0.
            // Degree d Multibrots have (d - 1)-fold rotational symmetry in the
            // parameter plane; the quindecic case has fourteen compact arms.
            Parallel.For(0, width, x =>
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    while (iterations < bailout && zr * zr + zi * zi <= 4.0)
                    {
                        ...
                        // Compute z^15 by repeated complex multiplication to avoid
